...
def test_data_coerced(index):
    ...
    create_or_update_documents(index, [a])
    d = fetch_document(index, "DoccyMcDocface")
...
def test_update(index_docs):
    ...
    assert fetch_document(index_docs, "0", _source=["annotations"]) == {}
    update_document(index_docs, "0", {"annotations": {"x": 3}})
    assert fetch_document(index_docs, "0", _source=["annotations"])["annotations"] == {"x": 3}
...
def test_update_by_query(index_docs):
    ...
    assert "subcat" not in fetch_document(index_docs, "3").keys()
...
def test_upload_without_identifiers(index):
    ...
    res = create_or_update_documents(index, [doc], fields={"title": "text", "text": "text", "date": "date"})
    assert res["successes"] == 1
    _assert_n(index, 1)

    # this doesnt identify duplicates
    res = create_or_update_documents(index, [doc])
    assert res["successes"] == 1
    _assert_n(index, 2)
...
def test_upload_with_explicit_ids(index):
    ...
    res = create_or_update_documents(index, [doc], fields={"title": "text", "text": "text", "date": "date"})
    assert res["successes"] == 1
    assert fetch_document(index, "1")["text"] == "text"
    ...
    assert res["successes"] == 1
    _assert_n(index, 1)
    assert fetch_document(index, "1")["title"] == "new title"


def test_upload_with_identifiers(index):
    doc = {"url": "http://", "text": "text"}
    res = create_or_update_documents(
    ...
    res = create_or_update_documents(index, [doc2])
    assert res["successes"] == 1
    _assert_n(index, 1)
    ...
    res = create_or_update_documents(index, [doc3])
    assert res["successes"] == 1
    _assert_n(index, 2)
    ...
    with pytest.raises(ValueError):
        create_or_update_documents(index, [doc4])
...
def test_invalid_adding_identifiers(index):
    ...
    doc = {"text": "text"}
    create_or_update_documents(index, [doc], fields={"text": "text"})
    refresh_index(index)

    # adding an identifier to an existing index should fail
    doc = {"url": "http://", "text": "text"}
    with pytest.raises(ValueError):
        create_or_update_documents(index, [doc], fields={"url": CreateDocumentField(type="keyword", identifier=True)})


def test_valid_adding_identifiers(index):
    doc = {"text": "text"}
    create_or_update_documents(index, [doc], fields={"text": CreateDocumentField(type="text", identifier=True)})

    # adding an additional identifier to an existing index should succeed if the index already has identifiers
    doc = {"url": "http://", "text": "text"}
    res = create_or_update_documents(index, [doc], fields={"url": CreateDocumentField(type="keyword", identifier=True)})
    ...
    res = create_or_update_documents(index, [doc1, doc2])
