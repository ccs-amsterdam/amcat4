...
from tests.tools import build_headers, check, get_json, set_auth
...
async def test_auth(client: AsyncClient, admin, index):
    ...
    with set_auth(AuthOptions.no_auth):
        ...
        assert (await client.get(f"/index/{index}", headers=build_headers(admin))).status_code == 200
        assert (await client.get(f"/index/{index}", headers=build_headers(unknown_user))).status_code == 200
    ...
    with set_auth(AuthOptions.allow_authenticated_guests):
        ...
        assert (await client.get(f"/index/{index}", headers=build_headers(unknown_user))).status_code == 403
        assert (await client.get(f"/index/{index}", headers=build_headers(admin))).status_code == 200
...
async def test_create_user(client: AsyncClient, user, writer, admin, username):
    ...
    assert (await client.delete(f"/users/{username}", headers=build_headers(writer))).status_code == 403
    assert (await client.delete(f"/users/{username}", headers=build_headers(username))).status_code == 204
    # (only) admin can delete everyone
    assert (await client.delete(f"/users/{user}", headers=build_headers(writer))).status_code == 403
    assert (await client.delete(f"/users/{user}", headers=build_headers(admin))).status_code == 204
...
async def test_modify_user(client: AsyncClient, user, writer, admin):
    ...
    await check(await client.put(f"/users/{user}", headers=build_headers(user), json={"role": "WRITER"}), 403)
    await check(await client.put(f"/users/{user}", headers=build_headers(admin), json={"role": "ADMIN"}), 200)
...
async def test_list_users(client: AsyncClient, index, admin, user):
    ...
    await check(await client.get("/users", headers=build_headers(user)), 403)
