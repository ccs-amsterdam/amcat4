...
def set_client_session_cookie(response: Response, exp: int, email: str):
...
async def oauth_login(request: Request, returnTo: str | None):
    ...
    if auth_url is None:
        raise ValueError("Server does not have an authentication provider set up")
...
async def oauth_logout(request: Request, returnTo: str | None):
